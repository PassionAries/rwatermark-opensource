...
export class XhsService implements OnModuleInit {
    async onModuleInit(){
        this.proxyurl = this.configService.get<string>('xhs.proxyUrl', '');
        console.log("XhsService onModuleInit",process.env.NODE_ENV);
        if(process.env.NODE_ENV=='local'){
            console.log("XhsService onModuleInit local skip");
            return null;
        }
    }
    // 构造请求头
 ...
 */
   extractId(url: string): string | null {
    ...
    getProxyUrl(url: string): {url:string,proxyHost:string} {
        // let match = url.match("https?:\/\/.+\.com");
        // if(match){
        //     return {url:url.replace(match[0],this.proxyurl),proxyHost:match[0]};
        // }
        return {url:url,proxyHost:""};
    }
        /**
         * 获取重定向后的 URL
         * @param url 原始 URL
        ...
        async get_curl(url: string, cookie: string = ''): Promise<string | null> {
                try {
                    ...
                    url = url.replace("http://xhslink.com",proxyurl);
                    }
                    if(url.includes("https://www.xiaohongshu.com")){
                    url = url.replace("https://www.xiaohongshu.com",proxyurl);
                    }
                    }
