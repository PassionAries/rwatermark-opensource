 ...
 * 
 * // 在index.service.ts中
 * private streamDownloadService = new StreamDownloadService(this.cacheDir);
 * 
 * async getFileInfo(url: string) {
 *   return await this.streamDownloadService.getFileInfo(url);
 * }
 * 
 * // 在index.controller.ts中
 * private streamController = new StreamDownloadController(this.rwatermarkService);
 * 
 * @Get("download")
 * async download(@Query() query: DownloadUrlDto, @Res() res: Response) {
 *   await this.streamController.downloadFile(query.url, res);
 * }
 */
